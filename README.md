...
- **Cmd+F** filter the current folder; **Cmd+Shift+G** go to a path; double-click empty space to go up.
...
- Multi-selection with Shift / Cmd; context-menu copy/move to another pane, compress to zip (collision-safe), move to Trash — all act on the whole selection.
- Drag files onto a folder row to move them (multi-file aware); inline rename; create folders and Markdown files.
...
- **Cmd+F** 过滤当前目录;**Cmd+Shift+G** 前往路径;双击空白返回上级。
- **类型快速过滤**:把面板收窄到文档 / 代码 / 数据 / 图片 / 压缩包 / 日志 / 构建依赖噪音(纯规则,不用 LLM)。
- Shift / Cmd 多选;右键复制/移动到另一面板、压缩为 zip(自动避免重名)、移到废纸篓——均对整组选中生效。
- 把文件拖到文件夹行即移动(支持多选);内联重命名;新建文件夹和 Markdown 文件。
