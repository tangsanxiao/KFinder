...
- **Agent Inbox**: a local review workbench that aggregates Claude / Codex sessions and git activity by project. It shows agent-session counts, uncommitted-change counts, risk findings, decisions / todos extracted from recent transcripts, and a commit-message draft.
...
- **Session Center**: read local Claude / Codex transcripts, group them by project, and search titles, projects, and transcript text without uploading data.
...
- **Agent Inbox**: 本地审查工作台,按项目聚合 Claude / Codex 会话和 git 活动。它展示 agent 会话数、未提交变更数、风险提示、从近期 transcript 抽取的决策 / 待办,并生成 commit message 草稿。
...
- **Session Center**: 读取本机 Claude / Codex transcript,按项目归组,支持标题、项目和 transcript 正文搜索,不上传数据。
