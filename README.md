...
XFinder is a macOS multi-pane file manager built for a personal workflow: viewing, comparing, copying, moving, renaming, and organizing files across several directories at the same time.

XFinder 是一个 macOS 多面板文件管理工具，主要服务于个人工作流：在多个目录之间同时查看、对比、复制、移动、重命名和整理文件。

The project started from a simple need: Finder is great for one folder at a time, but day-to-day work often involves multiple active directories. XFinder keeps those folders visible in one workspace so file management can happen without constantly switching Finder windows.

这个项目来自一个很具体的需求：Finder 很适合单个目录的管理，但日常工作里经常需要同时处理多个目录。XFinder 希望把这些目录放进同一个工作区，减少在多个 Finder 窗口之间来回切换。
...
- Supports creating folders and empty Markdown files from the pane empty-area context menu.
...
- Supports compressing selected files into a zip archive, with collision-safe archive names.
- Supports sorting by modified date and kind.
- Supports adaptive/resizable list columns for Name, Modified, Size, and Kind.
- Watches pane folders and expanded subfolders for external changes and refreshes automatically.
- Can reveal files or folders in native Finder.
- Can import currently open Finder windows as XFinder panes.
...
- 支持在面板空白区右键创建文件夹和空 Markdown 文档。
- 支持将文件拖拽移动到另一个面板。
- 支持通过右键菜单复制或移动文件到其它面板当前所在路径。
- 支持移动文件到废纸篓。
- 支持将选中文件压缩成 zip，并自动处理同名冲突。
- 支持按修改日期和文件种类排序。
- 支持 Name、Modified、Size、Kind 列的自适应和宽度调整。
- 支持监听面板目录和展开子目录的外部变化并自动刷新。
- 支持在原生 Finder 中显示文件或文件夹。
- 支持将当前已打开的 Finder 窗口导入为 XFinder 面板。

## Why Not Embed Finder?

macOS does not provide a stable public API for embedding native Finder windows inside another app. XFinder therefore implements its own file browser UI with SwiftUI/AppKit and uses Finder only for optional system integrations such as reveal and importing open Finder window paths.

## 为什么不直接嵌入 Finder？

macOS 没有提供稳定的公开 API，可以把原生 Finder 窗口直接嵌入到另一个应用里。因此 XFinder 使用 SwiftUI/AppKit 自己实现文件浏览界面，只在“在 Finder 中显示”和“导入当前 Finder 窗口路径”等场景中调用系统 Finder 能力。

## Architecture

XFinder is a Swift Package Manager macOS app with a SwiftUI interface and selected AppKit integrations.

XFinder 是一个基于 Swift Package Manager 的 macOS 应用，主要界面使用 SwiftUI 实现，并在窗口控制、Finder 集成、文件操作等位置使用 AppKit 能力。

Key modules:

- `XFinderApp.swift`: app entry point and scene setup.
...
- `scripts/build-app.sh`: builds `dist/XFinder.app`.
...
Download the latest `XFinder-*-macOS-*.zip` from the repository's Releases page, unzip it, then move `XFinder.app` to `/Applications`.

从仓库的 Releases 页面下载最新的 `XFinder-*-macOS-*.zip`，解压后将 `XFinder.app` 移动到 `/Applications`。
...
When using `Import Finder Windows`, macOS may ask for Automation permission so XFinder can read the paths of currently open Finder windows.
...
- `release/XFinder-<version>-macOS-<arch>.zip`
- `release/XFinder-<version>-macOS-<arch>.zip.sha256`
...
- Drag/drop is focused on moving files between XFinder panes.
- Automated coverage focuses on pure/file-operation logic; GUI behavior still needs manual verification.
...
- 拖拽能力主要聚焦于 XFinder 面板之间的文件移动。
- 自动化测试主要覆盖纯逻辑和文件操作，GUI 行为仍需要人工验证。

## Thanks

Thanks to [QSpace](https://qspace.awehunt.com/zh-cn/index.html#). XFinder referenced and learned from some of QSpace's product and interaction design during exploration. If you need a mature and stable multi-pane file manager, QSpace is recommended.

## 感谢

感谢 [QSpace](https://qspace.awehunt.com/zh-cn/index.html#)。XFinder 在设计探索过程中参考和借鉴了 QSpace 的部分产品与交互设计。如果你需要成熟稳定的多面板文件管理工具，推荐优先使用 QSpace。

## Project Status

XFinder is currently a usable personal tool and a GitHub-ready early release. It is suitable for experimentation and small-scope personal use, but should still be treated as an early-stage macOS file manager.

## 项目状态

XFinder 目前是一个基本可用的个人工具，也已经具备上传 GitHub 和进行早期分发的基础。它适合实验和小范围个人使用，但仍应被视为早期阶段的 macOS 文件管理工具。

## License

XFinder is released under the MIT License. See `LICENSE` for details.

## 许可证

XFinder 使用 MIT License 发布，详情见 `LICENSE` 文件。
