...
- Canonical skill library + symlink consolidation: "Consolidate into library" moves a skill into one library folder (default `~/Skills`, configurable in Settings) and replaces each agent's writable copy with a symlink — a single source of truth, so editing once applies everywhere and drift can't recur. Distributing a consolidated skill to another agent links it (stays single-source). The detail panel marks consolidated skills and the catalog shows whether each copy is a symlink.
